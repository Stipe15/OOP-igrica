...
class WorldMap extends WorldGeneral {
  ...
  addSpriteLayer(layer) {
    ...
    // Ako već postoji layer s tim imenom, izbriši stari i dodaj novi
    if (this._spriteLayers[layer.name] !== undefined) {
      delete this._spriteLayers[layer.name];
    }
...
class Layer extends WorldGeneral {
  ...
  collectTiles(tilesets) {
    ...
    for (let i = 0; i < ids.length; i++) {
      if (ids[i] > 0) {
        // Makni flip bitove (Tiled koristi 0x1FFFFFFF masku za tileId)
        let cleanId = ids[i] & 0x1FFFFFFF;
        this.tiles[cleanId] = new Tile(cleanId, this.tilewidth, this.tileheight);
      }
    }
   ...
   */
  customProperty(name) {
    // List of standard Layer properties that should not be treated as custom
    const standardProps = [
      "id", "name", "type", "used", "height", "width", "data", "properties", "visible",
      "tilewidth", "tileheight", "tiles", "x", "y", "opacity"
    ];
    // Only treat as custom if not a standard property and exists directly
    if (this.hasOwnProperty(name) && !standardProps.includes(name)) return this[name];

    // Then, check in the properties array (for custom properties)
